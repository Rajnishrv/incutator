...
# Load background image (same size as webcam frame will be used)
image_path = "background.jpg"  # replace with your image name
background_image = cv2.imread("IMG_2018_11_22_122956.jpg")

# Webcam for ball tracking
...
while True:
    ...
    resized_image = cv2.resize(background_image, (width, height))

    cell_w = width // cols
    cell_h = height // rows

    # Draw grid and current state
    draw_grid_and_marks(resized_image, cell_w, cell_h)
    ...
    winner = check_winner()
    if winner:
        cv2.putText(resized_image, f"{winner} Wins!", (50, height // 2),
                    cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 255), 4)

    # Show game screen on projector
    cv2.imshow("Game - Move to Projector", resized_image)
